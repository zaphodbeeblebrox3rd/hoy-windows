# Parse Website for Plain-Text Content
# Can be used to search for a string, or to run as a scheduled task and alert by email if there are any changes
# Author: Eric Hoy
# Date: 2021-10-04
# Modified: 2022-07-11

# Vars that can be changed
# Make sure to set this one for the external xml file
$variables_file = "\\harbor.uchicago.edu\sscs-server\Scripts\variables\sscs.xml"

# These are only for cases where you decide not to use an external data file
#$website_url="https://helpx.adobe.com/enterprise/kb/provisioning-toolkit-enterprise.html"
#$keyword_string="lastModifiedDate"
#$value_to_match='2021-11-13T13:35:54.119N'
#$smtp_relay='smtp.myuniversity.edu'
#$email_alert_targets='me@myuniversity.edu'
#$email_source_domain='myuniversity.edu'



# You can leave these values at the default
###########################################################
...
if (Test-Path -Path $variables_file) {
   ...
   $website_url = $sscs.sscs.website_monitoring.url
   $keyword_string = $sscs.sscs.website_monitoring.keyword_string
   $value_to_match = $sscs.sscs.website_monitoring.value_to_match
   $smtp_relay = $sscs.sscs.website_monitoring.smtp_relay
   $email_alert_targets = $sscs.sscs.website_monitoring.smtp_relay
   $email_source_domain = $sscs.sscs.website_monitoring.email_source_domain
   }

$email_from_name=($computer_name+'@'+$email_source_domain)

$line_separator="**********************************************************"
$email_subject="$computer_name has identified a changed in monitored website $website_url"
#########################################################################################


#Send an email alert
function EmailAlert($body,$targets,$subject,$relay,$from) {
    foreach ($target in $targets) {
        Write-Host ("Emailing: "+$target)
        Send-MailMessage -From "$from" -To "$target" -Subject "$subject" -Body "$body" -Priority Normal -SmtpServer "$relay"
        }
    }

$webpage_content=$(curl $website_url | select-object -Property Content) #grab the text from the specified URL

$webpage_lines=$($webpage_content -split "`r?`n") #split lines into a list variable that can be piped

$matching_output=$($webpage_lines | select-string -pattern $keyword_string) # get the content of the specific line(s) of interest

#We need a string for comparison but do not attempt to convert a null into a string
if ($matching_output -ne $null) {
    $matching_line=$matching_output.ToString()
    }

#capture strings to compose a message body or to show progress if being run manually
Write-Host ("Searching for this keyword: "+$keyword_string)
if ($matching_output -ne $null) {
    Write-Host ("Found this matching line: "+$matching_line)
    Write-Host ""
    Write-Host ("Comparing against specified value to match")
    if ($matching_line -notlike "*$value_to_match*"){
        $mismatch_message=($matching_line+" does not seem to contain: "+$value_to_match)
        $message_body=($email_subject+"`r"+"$mismatch_message")
        write-host $message_body
        EmailAlert $message_body $email_alert_targets $email_subject $smtp_relay $email_from_name
        }
    else {
        #no email, just a message if run manually
        Write-Host "All is good - no changes to this value!"
        }
    }
else {
    $message_body=("No matching line found on "+$website_url)
    Write-Host $message_body
    EmailAlert $message_body $email_alert_targets $email_subject $smtp_relay $email_from_name
    }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQ